...
def read_orbit_data(
    ...
    """
    Reads the orbit data from the NICER fits files.
    """
    ##### Import of data
    # Data location is just a folder on my computer
    if dir_name is None:
        dir_name = "/home/cephadrius/Desktop/test_folder/NICER_data/NICER_obs_orbit/"

    if file_name is None:
        file_name = "ni4202470103"

    # fits.open is used to read the fits files
    orb_fits = fits.open(f"{dir_name}/{file_name}.orb")
    att_fits = fits.open(f"{dir_name}/{file_name}.att")
    ...
    ax.legend()
    #plt.show()
    fig.savefig(f"../figures/{DataName}_OrbitPlot_GSM.png")
    plt.close("all")
